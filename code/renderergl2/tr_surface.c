...
static void RB_SurfaceVertsAndIndexes( int numVerts, srfVert_t *verts, int numIndexes, glIndex_t *indexes, int dlightBits, int pshadowBits)
{
    ...
    if ( tess.shader->vertexAttribs & ATTR_TEXCOORD )
    {
        ...
        for ( i = 0 ; i < numVerts ; i++, dv++, texCoords += NUM_TEX_COORDS * 2 )
    ...
    for ( i = 0 ; i < MAXLIGHTMAPS ; i++ )
    {
        ...
        //if ( tess.shader->vertexAttribs & ( ATTR_LIGHTCOORD + i ) ) // FIXME BOE
        {
            ...
            for ( j = 0 ; j < numVerts ; j++, dv++, lightCoords += NUM_TEX_COORDS * 2 )
                VectorCopy2(dv->lightmap[i], lightCoords);
        }
    }
...
static void RB_SurfaceGrid( srfBspSurface_t *srf ) {
    ...
    while ( used < lodHeight - 1 ) {
        ...
        for ( i = 0 ; i < MAXLIGHTMAPS ; i++ ) {
            lightCoords[i] = tess.texCoords[numVertexes][1 + i];
        }
